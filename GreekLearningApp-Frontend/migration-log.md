...
- Wrapper adoption pass:
  - Replaced direct MUI `Button`, `Fab`, and `TextField` imports across key app and feature pages with `@/components/ui` wrappers.
  - Replaced hardcoded onboarding option border color with theme token.
- Completion pass:
  - Replaced reader audio direct MUI icon button usage with `@/components/ui/IconButton`.
  - Replaced lesson completion direct MUI checkbox usage with `@/components/ui/Checkbox`.
